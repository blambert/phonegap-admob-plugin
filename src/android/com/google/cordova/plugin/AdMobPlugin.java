...
/**
 * This class represents the native implementation for the AdMob Cordova plugin. This plugin can be used to request AdMob ads natively via the Google AdMob SDK. The Google AdMob SDK is a dependency for this plugin.
 */
public class AdMobPlugin extends CordovaPlugin {
     ...
     * @return A PluginResult representing the result of the provided action. A status of INVALID_ACTION is returned if the action is not recognized.
     */
    @Override
    ...
    /**
     * Parses the create banner view input parameters and runs the create banner view action on the UI thread. If this request is successful, the developer should make the requestAd call to request an ad for the banner.
     *
     * @param inputs
     *            The JSONArray representing input parameters. This function expects the first object in the array to be a JSONObject with the input parameters.
    ...
    private synchronized void createBannerView(final String publisherId, final AdSize adSize, final CallbackContext callbackContext) {
     ...
     *
     * document.addEventListener('onReceiveAd', function()); document.addEventListener('onFailedToReceiveAd', function(data)); document.addEventListener('onPresentScreen', function()); document.addEventListener('onDismissScreen', function()); document.addEventListener('onLeaveApplication', function());
     */
